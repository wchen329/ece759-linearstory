...
namespace linearstory
{
			...
			virtual void Decompose() 
			{
				...
				#pragma omp for
					for(size_t dim_offset = 0; dim_offset < dim_pvt; ++dim_offset)
					{
						...
						U[dim_offset + dim_pvt * dim_offset] = S[0];
	...
	//						MatEcho<DataType>(S, dim_S, dim_S);	
						...
						DataType* uptr = U + (dim_offset * dim_pvt) + dim_offset + 1;
	...
	//						MatEcho<DataType>(hopptr, dim_S - 1, dim_S - 1);	
	//						MatEcho<DataType>(L, dim_pvt, dim_pvt);	
	//						MatEcho<DataType>(U, dim_pvt , dim_pvt);	
						//	assert(dim_offset != 0);
						#endif
